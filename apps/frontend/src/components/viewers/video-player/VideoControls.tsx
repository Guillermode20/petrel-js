...
import type { VideoPlayerState, VideoControls } from './types'
...
export function VideoControlBar({ state, controls, className }: VideoControlsProps) {
    ...
    if (transcodeJob?.status === 'processing') {
        return (
            <div className={cn('absolute inset-0 flex items-center justify-center bg-black/80', className)}>
                <div className="flex flex-col items-center gap-4 text-center">
                    <Loader2 className="h-8 w-8 animate-spin text-primary" />
                    <div>
                        <p className="text-sm font-medium">Preparing video...</p>
                        <p className="text-xs text-muted-foreground">
                            Transcoding: {transcodeJob.progress}%
                        </p>
                    </div>
                        ...
                        <DropdownMenuContent align="end" className="w-48">
                            ...
                            <DropdownMenuSub>
                                ...
                                <DropdownMenuSubContent>
                                    <DropdownMenuRadioGroup
                                        value={quality}
                                        onValueChange={controls.setQuality}
                                    >
                            ...
                            {audioTracks.length > 1 && (
                                <DropdownMenuSub>
                                    <DropdownMenuSubTrigger>
                                        Audio: {audioTracks[audioTrack]?.title ?? audioTracks[audioTrack]?.language ?? 'Track 1'}
                                    </DropdownMenuSubTrigger>
                                    <DropdownMenuSubContent>
                                        <DropdownMenuRadioGroup
