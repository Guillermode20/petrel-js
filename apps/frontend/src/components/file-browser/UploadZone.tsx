		...
		<div className="space-y-1">
			<div className="flex items-center justify-between gap-2">
				<div className="flex min-w-0 flex-1 items-center gap-2">
					{status === "completed" && <CheckCircle className="h-4 w-4 shrink-0 text-green-500" />}
				...
				<div className="flex items-center gap-1">
					...
					{(status === "pending" || status === "uploading") && onCancel && (
						<Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onCancel(file)}>
							<X className="h-3 w-3" />
						</Button>
					)}
					{(status === "completed" || status === "error") && onDismiss && (
						<Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => onDismiss(file)}>
							<X className="h-3 w-3" />
						</Button>
					)}
				</div>
