...
export function ContextMenuClipboardProvider({ children }: { children: ReactNode }): ReactNode {
    ...
    const actions = useMemo<ContextMenuClipboardActions>(() => ({ setItems, clear }), [clear, setItems]);
...
export function useContextMenuClipboardState(): ContextMenuClipboardState {
    ...
    if (!state) {
        throw new Error("useContextMenuClipboardState must be used within a ContextMenuClipboardProvider");
    }
    return state;
}
...
export function useContextMenuClipboardActions(): ContextMenuClipboardActions {
    ...
    if (!actions) {
        throw new Error("useContextMenuClipboardActions must be used within a ContextMenuClipboardProvider");
    }
    return actions;
}
