...
import type { VideoMetadata, TranscodeJob, VideoTrack, Subtitle } from '@petrel/shared';
import { db } from '../../db';
import { files, transcodeJobs, videoTracks, subtitles } from '../../db/schema';
import {
  probeFile,
  parseVideoMetadata,
...
export class VideoService {
  ...
  async analyzeFile(
    filePath: string
  ): Promise<{ metadata: VideoMetadata; assessment: TranscodeAssessment; probeResult: FFProbeResult }> {
    const probeResult = await probeFile(filePath);
  ...
  async saveVideoTracks(fileId: number, probeResult: FFProbeResult): Promise<void> {
    const tracksToInsert = probeResult.streams
      .filter((s) => s.codec_type === 'video' || s.codec_type === 'audio' || s.codec_type === 'subtitle')
      .map((stream) => ({
        fileId,
