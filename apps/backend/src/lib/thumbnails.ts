...
import { extractFrame, generateSpriteSheet, probeFile } from './ffmpeg';
...
export async function generateVideoThumbnail(
  ...
  await Bun.file(tempFramePath).delete().catch(() => null);
...
export async function generateVideoSprite(
  ...
  if (await fileExists(spriteAbsolute)) {
    const existingMeta = await Bun.file(spriteMetaAbsolute).json().catch(() => null);
    if (existingMeta) {
  ...
  await Bun.file(tempSpritePath).delete().catch(() => null);
...
export async function generateAllImageThumbnails(
  ...
  await Promise.all(
    sizes.map((size) => generateImageThumbnail(sourceAbsolutePath, fileId, size))
  );
}
...
export async function generateAllVideoThumbnails(
  ...
  await Promise.all(
    sizes.map((size) =>
      generateVideoThumbnail(sourceAbsolutePath, fileId, size, duration)
    )
  );
}
