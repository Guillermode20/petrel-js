...
async function extractAudioSamples(
	...
	const args = [
		...
		"1",
		"-filter:a",
		`aresample=8000,asetnsamples=n=${sampleCount}:p=0`,
		"-f",
		"s16le",
		"-acodec",
		"pcm_s16le",
...
async function getDuration(inputPath: string): Promise<number> {
	const args = ["-v", "quiet", "-print_format", "json", "-show_format", inputPath];
...
export async function generateWaveformData(
	...
	if (await fileExists(waveformDataPath)) {
		const existingData = await Bun.file(waveformDataPath)
			.json()
			.catch(() => null);
		if (existingData) {
...
export async function generateWaveformImage(
	...
	const args = [
		...
		sourceAbsolutePath,
		"-filter_complex",
		`aformat=channel_layouts=mono,showwavespic=s=${width}x${height}:colors=#a668fc`,
		"-frames:v",
		"1",
		"-y",
		waveformImagePath,
