...
export function assessTranscodeNeeds(probeResult: FFProbeResult): TranscodeAssessment {
	...
	const audioWebCompatible =
		audioCodec === null || WEB_COMPATIBLE_AUDIO_CODECS.includes(audioCodec.toLowerCase());
...
export async function extractFrame(options: ExtractFrameOptions): Promise<void> {
	...
	const args = ["-ss", timestamp.toString(), "-i", inputPath, "-vframes", "1", "-y"];
...
export async function generateSpriteSheet(options: GenerateSpriteOptions): Promise<void> {
	...
	const args = [
		...
		inputPath,
		"-vf",
		`fps=1/${interval},scale=${thumbWidth}:${thumbHeight},tile=${columns}x${rows}`,
		"-frames:v",
		"1",
		"-y",
		outputPath,
...
export async function extractSubtitle(options: ExtractSubtitleOptions): Promise<void> {
	...
	const args = ["-i", inputPath, "-map", `0:${streamIndex}`, "-c:s", "webvtt", "-y", outputPath];
...
function getTranscodeParams(quality: TranscodeOptions["quality"]): {
	width: number;
	height: number;
	bitrate: string;
} {
...
export async function transcodeToHLS(options: TranscodeOptions): Promise<string> {
	...
	const args = [
		...
		params.bitrate,
		"-bufsize",
		`${Number.parseInt(params.bitrate, 10) * 2}M`,
		"-c:a",
		"aac",
		"-b:a",
		"128k",
	...
	const stdoutReader = async (): Promise<void> => {
		...
		while (true) {
			...
			if (timeMatch?.[1] && duration > 0 && onProgress) {
