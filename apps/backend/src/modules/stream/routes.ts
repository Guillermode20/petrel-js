...
export const streamRoutes = new Elysia({ prefix: '/api/stream' })
  ...
  .post(
    '/:fileId/prepare',
    async ({ params, set, user }): Promise<ApiResponse<{ jobId: number | null; ready: boolean }>> => {
      if (!canRead(user)) {
        set.status = 401;
        return { data: null, error: 'Unauthorized' };
      }
  ...
  .get(
    '/:fileId/subtitles',
    async ({ params, set, user }): Promise<ApiResponse<Array<{ id: number; language: string; title: string | null }>>> => {
      if (!canRead(user)) {
        set.status = 401;
        return { data: null, error: 'Unauthorized' };
      }
  ...
  .get(
    '/:fileId/tracks',
    async ({ params, set, user }): Promise<ApiResponse<Array<{ index: number; type: string; codec: string; language: string | null; title: string | null }>>> => {
      if (!canRead(user)) {
        set.status = 401;
        return { data: null, error: 'Unauthorized' };
      }
